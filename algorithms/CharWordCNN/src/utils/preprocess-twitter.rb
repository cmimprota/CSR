...
#
# (from https://nlp.stanford.edu/projects/glove/, adapted for ETH Computational Intelligence Lab project)
...
def tokenize input
	...
	input = input
		...
		.gsub(/<3/,"<HEART>") # .gsub(/[-+]?[.\d]*[\d]+[:,.\d]*/, "<NUMBER>") #	MOVED FOR CIL
		...
		.gsub(/[-+]?[.\d]*[\d]+[:,.\d]*/, " <NUMBER> ") # MOVED FOR CIL + ADDED WHITESPACES
		...
		.gsub(/'/, "") # ADDED FOR CIL
#		.gsub(/([^a-z0-9()<>'`\-]){2,}/){ |word| # REMOVED FOR CIL
